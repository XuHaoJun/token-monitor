...
- **Predictive quota alerts** — instead of just showing what's left, Token Monitor tracks each quota window's burn rate (15m / 1h / 24h regression over the normalized limit history) and predicts whether the quota will run out before its reset. Session/window rows in the AI Tool Limits view show burn rate, ETA and the projected margin before reset; the tray icon shows a yellow **WARNING** / red **CRITICAL** badge (and a tooltip line) when any account is projected to exhaust before reset. Forecast history is kept locally in `quota-forecast-history.json` (14 days, atomic writes), works identically in local and hub-sync modes, and never polls providers or touches the hub protocol.
