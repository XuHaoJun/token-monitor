...
The fork adds a consumer-side forecast layer on top of the existing limits abstraction: `src/shared/quotaForecast.js` (pure: series identity, generation/local-segment segmentation, OLS burn rates, ETA, risk, global aggregation), `src/electron/quotaForecastStore.js` (atomic JSON history under `userData`), and `src/electron/quotaForecastController.js` (dedupe, hysteresis, staleness re-evaluation, wiring). The controller is fed from the widget's normal stats flow (`sendPush`) and never polls providers, never changes `limitsRefreshMs`, and never touches the hub protocol; in sync/host mode it consumes the same hub-aggregated `stats.limits` the renderer sees. Tray risk icons (`risk-warning`/`risk-critical`) are generated by the renderer canvas pipeline and shipped through the existing `tray:setIcons` channel; they must never be macOS template-marked. Settings live under `predictiveQuota*` (`TOKEN_MONITOR_PREDICTIVE_QUOTA_ALERTS` / `TOKEN_MONITOR_PREDICTIVE_QUOTA_TRAY_INDICATOR` env overrides). Every failure path is fail-open: forecast errors degrade to `unknown`, never a crash.
